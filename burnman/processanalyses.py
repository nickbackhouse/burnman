...
def fit_composition(fitted_elements, composition, compositional_uncertainties,
    ...
    endmember_constraints = lambda site_occ: [{'type': 'ineq',
                                               'fun': lambda x, eq=eq:
                                                   eq.dot(x)}
                                              for eq in site_occ]
...
def compute_and_set_phase_compositions(assemblage, verbose=False):
    for phase in assemblage.phases:
        if isinstance(phase, SolidSolution):
            ...
            fitted_elements = phase.fitted_elements
            composition = phase.composition
            compositional_uncertainties = phase.compositional_uncertainties
            ...
            # Convert uncertainties in amounts
            # into uncertainties in proportions
            sum_popt = sum(popt)
...
def assemblage_affinity_misfit(assemblage, reuse_reaction_matrix=True):
    ...
    if len(reaction_matrix) == 0:
        ...
        n_mbrs = sum([phase.n_endmembers if isinstance(phase, SolidSolution)
                      else 1 for phase in assemblage.phases])
        ...
        for phase in assemblage.phases:
            if isinstance(phase, SolidSolution):
                Cov_mu[i:i+phase.n_endmembers,
                       i:i+phase.n_endmembers] = (phase.gibbs_hessian).dot(phase.molar_fraction_covariances).dot(phase.gibbs_hessian.T) # the hessian is symmetric, so transpose only taken for legibility...
                dmudPT[i:i+phase.n_endmembers] = np.array([phase.partial_volumes,
                                                           -phase.partial_entropies]).T
